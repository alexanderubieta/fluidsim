...
class StaggeredGrid {
  public:
    ...
    void ParticlesToGrid(const std::vector<Particle>& particles){
        ...
        // Before setting boundaries, store these in fu,fv,fw
        fu.SetEqualTo(su);
        fv.SetEqualTo(sv);
        fw.SetEqualTo(sw);
    ...
    const Eigen::Vector3d Advect(Eigen::Vector3d pos, double t){
        Eigen::Vector3d n = pos + t*triInterpolate(pos, su, sv, sw);
        if(n[0] == 0){
            n[0] += 1;
        }
        if(n[0] == nx() - 1){
    ...
    Eigen::Vector3d GridToParticle(double ratio, const Particle& particle){
        Eigen::Vector3d oldVel = triInterpolate(particle.pos,fu,fv,fw);
        Eigen::Vector3d newVel = triInterpolate(particle.pos,su,sv,sw);
        return (ratio*(particle.vel - oldVel)) + newVel;
    }
  private:
    Eigen::Vector3d triInterpolate(const Eigen::Vector3d& pos, 
    const Array3D<double>& cu,const Array3D<double>& cv, const Array3D<double>& cw){
        Eigen::Vector3d pshift = pos - shift_yz_;
        Eigen::Vector3d pshift1 = pos - shift_xz_;
        Eigen::Vector3d pshift2 = pos - shift_xy_;
        ...
        double ru = (1-bary[0])*(1-bary[1])*(1-bary[2])*cu(cell[0],cell[1],cell[2]) + 
        (1-bary[0])*(1-bary[1])*(bary[2])*cu(cell[0],cell[1],cell[2]+1) + 
        (1-bary[0])*(bary[1])*(1-bary[2])*cu(cell[0],cell[1]+1,cell[2]) + 
        (1-bary[0])*(bary[1])*(bary[2])*cu(cell[0],cell[1]+1,cell[2]+1) + 
        (bary[0])*(1-bary[1])*(1-bary[2])*cu(cell[0]+1,cell[1],cell[2]) +
        (bary[0])*(1-bary[1])*(bary[2])*cu(cell[0]+1,cell[1],cell[2]+1) + //-
        (bary[0])*(bary[1])*(1-bary[2])*cu(cell[0]+1,cell[1]+1,cell[2]) + 
        (bary[0])*(bary[1])*(bary[2])*cu(cell[0]+1,cell[1]+1,cell[2]+1);

        cell = floor(pshift1 - lc,dx);
        bary = weights(pshift1/dx,cell);
        double rv = (1-bary[0])*(1-bary[1])*(1-bary[2])*cv(cell[0],cell[1],cell[2]) + 
        (1-bary[0])*(1-bary[1])*(bary[2])*cv(cell[0],cell[1],cell[2]+1) + 
        (1-bary[0])*(bary[1])*(1-bary[2])*cv(cell[0],cell[1]+1,cell[2]) + 
        (1-bary[0])*(bary[1])*(bary[2])*cv(cell[0],cell[1]+1,cell[2]+1) + 
        (bary[0])*(1-bary[1])*(1-bary[2])*cv(cell[0]+1,cell[1],cell[2]) +
        (bary[0])*(1-bary[1])*(bary[2])*cv(cell[0]+1,cell[1],cell[2]+1) + //-
        (bary[0])*(bary[1])*(1-bary[2])*cv(cell[0]+1,cell[1]+1,cell[2]) + 
        (bary[0])*(bary[1])*(bary[2])*cv(cell[0]+1,cell[1]+1,cell[2]+1);

        cell = floor(pshift2 - lc,dx);
        bary = weights(pshift2/dx,cell);
        double rw = (1-bary[0])*(1-bary[1])*(1-bary[2])*cw(cell[0],cell[1],cell[2]) + 
        (1-bary[0])*(1-bary[1])*(bary[2])*cw(cell[0],cell[1],cell[2]+1) + 
        (1-bary[0])*(bary[1])*(1-bary[2])*cw(cell[0],cell[1]+1,cell[2]) + 
        (1-bary[0])*(bary[1])*(bary[2])*cw(cell[0],cell[1]+1,cell[2]+1) + 
        (bary[0])*(1-bary[1])*(1-bary[2])*cw(cell[0]+1,cell[1],cell[2]) +
        (bary[0])*(1-bary[1])*(bary[2])*cw(cell[0]+1,cell[1],cell[2]+1) + //-
        (bary[0])*(bary[1])*(1-bary[2])*cw(cell[0]+1,cell[1]+1,cell[2]) + 
        (bary[0])*(bary[1])*(bary[2])*cw(cell[0]+1,cell[1]+1,cell[2]+1);
